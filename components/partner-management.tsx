...
import ColumnVisibilityPicker from "@/components/column-visibility-picker"
...
export default function PartnerManagement() {
  ...
  const columnFields = useMemo(
    () => [
      { id: "partner", label: "Opportunity" },
      { id: "industry", label: "Industry" },
      { id: "size", label: "Size" },
      { id: "region", label: "Region" },
      { id: "status", label: "Status" },
      { id: "customers", label: "Customers" },
      { id: "opportunities", label: "Partners" }, // Note: labeled as "Partners" in UI
  ...
  const defaultVisibleColumns = useMemo(
    () => ["partner", "industry", "size", "region", "status", "customers", "opportunities", "contacts", "template"],
    [],
  )

  const [visibleFields, setVisibleFields] = useState(() => {
    try {
      const storageKey = `qai:colvis:v1:user:partner-management:opportunities`
  ...
  useEffect(() => {
    if (sortConfig.key && !visibleFields[sortConfig.key as keyof typeof visibleFields]) {
      setSortConfig({ key: null, direction: "asc" })
    }
  }, [visibleFields, sortConfig.key])
  ...
  // Check if we should show the dynamic list creation button
  const shouldShowDynamicListButton = useMemo(() => {
    return (
  ...
  }, [isEditingListContent, isEditingDynamicList, isEditingViewContent, selectedListId, hasActiveFilters, searchTerm])

  // Check if search should be disabled (when viewing a dynamic list with search criteria and not in edit mode)
  const isSearchDisabled = useMemo(() => {
    return (
      selectedList?.type === "dynamic" &&
      selectedList?.searchTerm &&
      !isEditingDynamicList &&
      !isEditingListContent &&
      !isEditingViewContent
    )
  }, [selectedList, isEditingDynamicList, isEditingListContent, isEditingViewContent])

  // Check if filters should be disabled (when viewing a static list and not in edit mode)
  const isFiltersDisabled = useMemo(() => {
    return selectedList?.type === "static" && !isEditingDynamicList && !isEditingListContent && !isEditingViewContent
  }, [selectedList, isEditingDynamicList, isEditingListContent, isEditingViewContent])

  // Check which specific filter controls should be disabled
  const getFilterControlDisabled = useMemo(() => {
    if (isEditingDynamicList || isEditingListContent || isEditingViewContent) {
      return () => false // All controls enabled in edit modes
    }

    if (selectedList?.type === "dynamic" && selectedList?.filters) {
      const listFilters = selectedList.filters
      return (filterKey: keyof FilterState) => {
        // Disable if this filter was used to create the dynamic list (not "all")
        return listFilters[filterKey] !== "all"
      }
    }

    if (selectedList?.type === "static") {
      return () => true // All filter controls disabled for static lists
    }

    return () => false // All controls enabled for "All Opportunities"
  }, [selectedList, isEditingDynamicList, isEditingListContent, isEditingViewContent])
    ...
    if (isEditingListContent) {
      ...
      if (selectedList) {
        if (selectedList.type === "static" && selectedList.opportunityIds) {
          // For static lists, start with the specific opportunities
          opportunitiesToFilter = mockOpportunities.filter((opportunity) =>
            selectedList.opportunityIds!.includes(opportunity.id),
          )
        } else if (selectedList.type === "dynamic" && selectedList.filters) {
          // For dynamic lists, apply the saved filters and search term
          const listFilters = selectedList.filters
          ...
          opportunitiesToFilter = mockOpportunities.filter((opportunity) => {
            const matchesListSearch =
              !listSearchTerm ||
              opportunity.name.toLowerCase().includes(listSearchTerm.toLowerCase()) ||
              opportunity.code.toLowerCase().includes(listSearchTerm.toLowerCase())
          ...
          // For dynamic lists that aren't "All Opportunities", apply additional search/filters only if they're different from the saved ones
          if (selectedList.id !== "1") {
            // Apply additional search if current search is different from saved search
            if (searchTerm && searchTerm !== listSearchTerm) {
              opportunitiesToFilter = opportunitiesToFilter.filter(
                (opportunity) =>
                  opportunity.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                  opportunity.code.toLowerCase().includes(searchTerm.toLowerCase()),
              )
            ...
            // Apply additional filters only for filter values that are different from the saved ones
            opportunitiesToFilter = opportunitiesToFilter.filter((opportunity) => {
              return (
                (filters.status === "all" ||
                  filters.status === listFilters.status ||
                  opportunity.status === filters.status) &&
                (filters.type === "all" ||
                  filters.type === listFilters.type ||
                  opportunity.industry.toLowerCase() === filters.type.toLowerCase()) &&
                (filters.size === "all" || filters.size === listFilters.size || opportunity.size === filters.size) &&
                (filters.stage === "all" || filters.stage === listFilters.stage || opportunity.status === filters.stage)
              )
            })
          }
        }
      ...
      // Apply current search term and filters (for "All Opportunities")
      if (selectedList?.id === "1" || !selectedList) {
        ...
        opportunitiesToFilter = opportunitiesToFilter.filter((opportunity) => {
          return (
            ...
            (filters.type === "all" || opportunity.industry.toLowerCase() === filters.type.toLowerCase()) &&
            (filters.size === "all" || opportunity.size === filters.size) &&
            (filters.stage === "all" || opportunity.status === filters.stage)
          )
        })
      }
    }
  ...
  const performListSelect = (listId: string) => {
    ...
    // If it's a dynamic list, apply its filters and search term
    if (list && list.type === "dynamic" && list.filters) {
      setFilters(list.filters)
      if (list.searchTerm) {
        setSearchTerm(list.searchTerm)
      } else {
        setSearchTerm("")
      }
    } else {
      // If it's a static list or "All Opportunities", clear filters and search
      clearFilters()
      setSearchTerm("")
    }
  ...
  const handleCreateDynamicList = () => {
    const newList: SavedList = {
      ...
      value: 0,
      filters: { ...filters },
      searchTerm: searchTerm.trim() || undefined, // Only include search term if it exists
    }
        ...
        prev.map((list) =>
          list.id === editingDynamicListId
            ? {
                ...
                opportunityCount: filteredOpportunities.length,
                searchTerm: searchTerm.trim() || undefined, // Update search term
              }
            : list,
        ),
      )
  ...
  return (
    <TooltipProvider>
      <div className="p-6 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            {selectedList && (
              <div className="px-3 py-1 bg-purple-50 border border-purple-200 rounded-md">
                ...
                {/* Show search term badge for dynamic lists with search criteria */}
                {selectedList.type === "dynamic" && selectedList.searchTerm && (
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {/* Show search term badge for dynamic lists with search criteria */}
                                {list.type === "dynamic" && list.searchTerm && (
                        ...
                        <CardFooter className="py-2 px-3 flex justify-end">
                          ...
                          {list.id !== "1" && ( // Conditionally render share button for "All Opportunities"
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-7 px-2 text-muted-foreground hover:text-foreground hover:bg-purple-100"
                              onClick={(e) => {
                                e.stopPropagation()
                                handleOpenShareDialog(list.id, list.name)
                              }}
                            >
              ...
              <div className="flex items-center gap-2">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="outline"
                      className="flex items-center gap-2 bg-transparent text-sm h-7"
                ...
                <ColumnVisibilityPicker
                  ...
                  routeKey="partner-management"
                  entityType="partners"
                  fields={columnFields}
                  defaultVisibleIds={defaultVisibleColumns}
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    disabled={getFilterControlDisabled("status")}
                  >
                ...
                <div>
                  ...
                  <Select
                    ...
                    disabled={getFilterControlDisabled("type")}
                  >
                    <SelectTrigger className="text-sm">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    disabled={getFilterControlDisabled("size")}
                  >
                    <SelectTrigger className="text-sm">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    disabled={getFilterControlDisabled("stage")}
                  >
                    <SelectTrigger className="text-sm">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    disabled={getFilterControlDisabled("region")}
                  >
                    <SelectTrigger className="text-sm">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    disabled={getFilterControlDisabled("priority")}
                  >
                    <SelectTrigger className="text-sm">
                      <SelectValue />
                    </SelectTrigger>
        ...
        {/* Save Actions Row - Show dynamic list button when search term or filters are active */}
        {shouldShowDynamicListButton && (
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <CardDescription className="text-sm">{opportunity.code}</CardDescription>
                    </div>
                    <Checkbox
                      id={`opportunity-${opportunity.id}`}
                      checked={
                ...
                <CardContent className="pt-0">
                  {visibleFields.industry && (
                    <div className="mb-2">
                      ...
                      <p className="text-sm">{opportunity.industry}</p>
                    </div>
                  )}
                  {visibleFields.size && (
                    <div className="mb-2">
                  ...
                  {visibleFields.region && (
                    <div className="mb-2">
                      ...
                      <p className="text-sm">{opportunity.region}</p>
                    </div>
                  )}
                  {visibleFields.status && (
                    <div className="mb-2">
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-3">
                    ...
                    {visibleFields.contacts && (
                      <div className="text-center">
                        <div className="text-sm font-medium">{opportunity.contacts}</div>
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <Alert className="mb-4 border-blue-200 bg-blue-50">
              ...
              <AlertDescription className="text-blue-800 text-sm">
                Dynamic Lists automatically update as opportunities match your selected filters and search criteria.
              </AlertDescription>
            </Alert>
              ...
              <div className="space-y-2">
                ...
                <div className="p-3 bg-gray-50 rounded-lg border">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {Object.values(filters).every((value) => value === "all") && !searchTerm.trim() && (
                      <span className="text-xs text-gray-500 italic">
                        No filters or search criteria applied - all opportunities will be included
                      </span>
                    )}
                  </div>
                </div>
