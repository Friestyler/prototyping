...
- Switched the outermost shape from `SELECT DISTINCT ... JOIN products JOIN auto_tree` to `FROM customers WHERE ... AND EXISTS (SELECT 1 FROM products JOIN auto_tree ...)`. One row per customer is now guaranteed by construction rather than by after-the-fact `DISTINCT`. Same 168 rows.
