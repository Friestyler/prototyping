...
2. **Use the Qollabi schema as the source of truth.** The schema lives under `schema/qollabi-schema/`. When CSV column names don't match schema names, reconcile them and note the mapping in the requirement's report.
   ...
   - **Read [`schema/qollabi-schema/semantics.yaml`](../schema/qollabi-schema/semantics.yaml) before generating SQL.** It carries per-table invariants, per-column semantics (including `do_not_filter` flags), and named SQL patterns (e.g. `domein-tree`, `polistype-inside-domein`) that requirement `.sql` files compose. The patterns there are the canonical recipes — match the structure when a requirement fits a named pattern; cite the pattern by name in the requirement's report.
