...
- **Never hand-simulate a query result.** Every answer ships as exactly three files in `sql-results/<database-name>/` — `<name>.sql`, `<name>.csv`, `<name>.report.md` — with the CSV produced by running `qollabi-view.sql` + `<name>.sql` through DuckDB in a single connection. No `business-requirements/<name>.md` — the requirement text lives inside `.report.md`.
- **`.report.md` embeds the SQL in a fenced code block.** So non-technical readers without a SQL editor can see the query. When you update the `.sql`, re-paste it into the report's SQL section.
...
- **One requirement = one `.sql` + one `.csv` + one `.report.md`.** Iterate in place; no `-v2` / `-final` duplicates.
