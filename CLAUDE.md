...
- **Never hand-simulate a query result.** Every answer ships as `<name>.sql` + `<name>.csv` + `<name>.report.md` in `sql-results/<database-name>/`, with the CSV produced by executing the `.sql` through DuckDB.
